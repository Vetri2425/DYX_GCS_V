...
export function normalizeJoystickError(raw: unknown): JoystickErrorPayload {
  if (
    raw &&
    typeof raw === 'object' &&
    (raw as JoystickErrorPayload).type === 'joystick_error' &&
    typeof (raw as JoystickErrorPayload).code === 'string'
  ) {
    const event = raw as JoystickErrorPayload;
    return {
      type: 'joystick_error',
      code: event.code,
      message:
  ...
  if (raw && typeof raw === 'object' && typeof (raw as JoystickErrorPayload).code === 'string') {
    const code = (raw as JoystickErrorPayload).code as JoystickErrorCode;
    const message = (raw as JoystickErrorPayload).message;
    return {
      type: 'joystick_error',
      code,
